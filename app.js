(() => {
  ...
  const toggle = root.querySelector('#v19-uncovered');
  const uniformToggle = root.querySelector('#v19-uniform');
  const reflowToggle = root.querySelector('#v19-reflow');
  const ghostControl = root.querySelector('#v19-ghost-control');
  const variationLabel = root.querySelector('#v19-variation');
  const copyVariationButton = root.querySelector('#v19-copy-variation');
  ...
  function frame(option,index){
    ...
    article.innerHTML=`<div class="option-heading"><div class="option-copy"><strong>${option.title}</strong><span class="text-small">${option.note}</span></div><button type="button" class="btn export-slide">Download 1920×1080 PNG</button></div>`;
  ...
  function configureTechniqueGrid(tactic){
    ...
    const minimumSize=mode==='lane'?11:12;
    ...
    for(const columns of columnCandidates){
      ...
      if(!best||score>best.score) best={columns,rows,fontSizes,minimumFont,score};
    }
  ...
  function fitSlide(slide){
    ...
    const minimumSize=slide.dataset.option==='lanes'?11:12;
  ...
  function activateOption(optionId,focusTab=false){
    ...
    toggle.checked=state.uncoveredTechniques;
    uniformToggle.checked=state.uniformSizeAndText;
    reflowToggle.checked=state.calculateGhosts;
    ghostControl.hidden=optionId!=='weighted';
    copyVariationButton.textContent='Copy';
    update();
  }
  function update(){
    const panel=getOptionPanel();
    const state=optionStates.get(activeOptionId);
    if(!panel||!state) return;
    state.uncoveredTechniques=toggle.checked;
    state.uniformSizeAndText=uniformToggle.checked;
    state.calculateGhosts=activeOptionId==='weighted'&&reflowToggle.checked;
    reflowToggle.checked=state.calculateGhosts;
    const show=state.uncoveredTechniques;
    const reflow=state.calculateGhosts;
    panel.classList.toggle('uniform-mode',state.uniformSizeAndText);
    const variation=JSON.stringify(state);
    variationLabel.textContent=variation;
    variationLabel.setAttribute('aria-label',`Variation settings for ${options.find(option=>option.id===activeOptionId)?.title}: ${variation}`);
    copyVariationButton.dataset.copyText=variation;
    copyVariationButton.title='Copy variation object';
  ...
  async function copyVariation(){
    const text=copyVariationButton.dataset.copyText||variationLabel.textContent;
    let copied=false;
    try{
    ...
    copyVariationButton.textContent=copied?'Copied':'Copy failed';
    setTimeout(()=>{ copyVariationButton.textContent='Copy'; },1400);
  }
  let layoutFrame;
  ...
  uniformToggle.addEventListener('change',update);
  reflowToggle.addEventListener('change',update);
  copyVariationButton.addEventListener('click',copyVariation);
